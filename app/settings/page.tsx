...
type CsvDataset = "tasks" | "food" | "lifecycle" | "smoking";
...
export default function SettingsPage() {
  ...
  const dirty = {
    ...
    calories: !sameFields(settings, data.settings, ["defaultMealType", "aiFoodAnalysisEnabled", "requireAiConfirmation"]),
  ...
  return <div className="page-wrap">
    ...
    <div className="mb-5 grid gap-4 md:grid-cols-3">
      ...
      <Card className="p-5"><HardDrive size={19} className="text-acid" /><p className="mt-4 font-display font-bold">Localhost datastore</p><p className="mt-2 text-xs leading-5 text-zinc-600">Every save persists to the server-side data file. Supabase remains disconnected for this local version.</p></Card>
    </div>
    ...
    <div className="space-y-5">
      ...
      <Card className="p-5 md:p-6"><SectionHeader eyebrow="Calories" title="Meal capture behavior" dirty={dirty.calories} saving={saving === "calories"} save={() => void saveSection("calories", "/api/settings/calories", { defaultMealType: settings.defaultMealType, aiFoodAnalysisEnabled: settings.aiFoodAnalysisEnabled, requireAiConfirmation: settings.requireAiConfirmation })} /><div className="grid gap-4 lg:grid-cols-3"><label><span className="mb-1.5 block text-xs text-zinc-500">Default Meal Type</span><select className={inputClass} value={settings.defaultMealType} onChange={(e) => change("defaultMealType", e.target.value as SystemSettings["defaultMealType"])}><option value="auto">Auto Detect</option><option value="breakfast">Breakfast</option><option value="lunch">Lunch</option><option value="dinner">Dinner</option><option value="snack">Snack</option></select></label><Toggle checked={settings.aiFoodAnalysisEnabled} onChange={(value) => change("aiFoodAnalysisEnabled", value)} label="Enable AI Food Analysis" description="Controls the photo analysis button and API." /><Toggle checked={settings.requireAiConfirmation} onChange={(value) => change("requireAiConfirmation", value)} label="Require AI Confirmation" description="When off, a valid AI result is saved immediately." /></div></Card>
